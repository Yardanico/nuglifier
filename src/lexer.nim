...
import std / [hashes, strutils, parseutils]
import compiler / [options, msgs, platform, idents, nimlexbase, llstream, wordrecg, lineinfos, pathutils] 
  ...
  while true:
    ...
    of '#':
      ...
      if L.buf[pos+1] == '[':
        ...
        tokenBegin(tok, pos)
        while L.buf[pos] notin {CR, LF, nimlexbase.EndOfFile}:
          when true: tok.literal.add L.buf[pos]
          inc(pos)
        tokenEndIgnore(tok, pos+1)
        when true:
          tok.commentOffsetB = L.offsetBase + pos + 1
